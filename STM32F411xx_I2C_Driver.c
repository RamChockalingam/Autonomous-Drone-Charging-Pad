...
void I2C_Init(I2C_Handle_t *pI2CHandle)
{
	...
	//enabling PE bit to enable ACK bit properly
	...
	//program the devices own address
	...
	//disabling PE bit to enable trise bits properly
	//pI2CHandle->pI2Cx->CR1 &= ~(1 << 0);
	//TRISE Configuration
	if(pI2CHandle->I2C_Config.I2C_SCLSpeed <= I2C_SCL_SPEED_SM)
	{
...
void I2C_MasterSendData(I2C_Handle_t *pI2CHandle, uint8_t *pTxbuffer, uint32_t Len, uint8_t SlaveAddr,uint8_t sr)
{
	...
	//3. Send the address of the slave with r/n w bit set to w(0) (total 8 bits )
	...
	while(Len > 0)
	{
		...
		*((volatile uint8_t*)&pI2CHandle->pI2Cx->DR) = *pTxbuffer;
		pTxbuffer++;
		Len--;
	}
...

{
	...
	while( ! I2C_GetFlagStatus(pI2Chandle->pI2Cx,I2C_FLAG_SB) );
	//3. send the address of the slave with r/w bit set to R(1) (8bits)
...
static void I2C_MasterHandleTXEInterrupt(I2C_Handle_t *pI2CHandle)
{
	if(pI2CHandle->TxLen >0)
	{
		...
		*((volatile uint8_t*)&pI2CHandle->pI2Cx->DR) = *(pI2CHandle->pTxBuffer);
...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle)
{
	...
	{
		// STOPF Flag is set
		//Clear the STOPF (i.e 1) read SR1 and write to CR1
	...
	if( temp1 && temp2 && temp3)
	{
		if(pI2CHandle->pI2Cx->SR2 & (1 <<I2C_SR2_MSL) )
		{
			//TXE Flag is set indicating DR is empty
